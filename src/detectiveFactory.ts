...
function getFilename(code: string, options?: ParserOptions): string {
  ...
  const hasTypeScript = /:\s*\w+\s*[=;]|interface\s+\w+|type\s+\w+\s*=|enum\s+\w+/.test(code);
 ...
 */
export function detectiveFactory(code: string | ASTNode, options?: ParserOptions, cjs = false): Dependency[] {
  ...
  function processNode(node: ASTNode): void {
    switch (node.type) {
      case 'ImportDeclaration': {
        const dep: Dependency = { type: 'module' };
        ...
        for (let i = 0; i < importNode.specifiers.length; i++) {
          ...
          switch (specifiersType) {
            ...
            case 'ImportSpecifier':
              ...
              dep.members.push({
                name: specifiersNode.imported?.name || specifiersNode.local.name,
        ...
        if (cjs) {
          ...
          if (callNode.callee?.name === 'require') {
            ...
            if (args.length > 0 && args[0].type === 'Literal') {
              const dep: Dependency = { type: 'commonjs' };
